...
extern MenuAction airactions[];
extern size_t airactions_count;
extern MenuAction gsactions[];
...
static void on_rec_enabled(const char *value) /* start/stop recording now */
{
    ...
    if (on)
        DVR_LIVE(dvr_start_all(), "dvr_start_all()%s", "");
    else
        DVR_LIVE(dvr_stop_all(), "dvr_stop_all()%s", "");
}
...
static void on_rec_fps(const char *value) /* raw recorder framerate */
{
    ...
    if (fps > 0)
        DVR_LIVE(dvr_set_raw_fps(fps), "dvr_set_raw_fps(%d)", fps);
}
...
static void on_dvr_reenc_fps(const char *value)
{
    int fps = value ? atoi(value) : 0;
    if (fps > 0)
        DVR_LIVE(dvr_reenc_set_fps(fps), "dvr_reenc_set_fps(%d)", fps);
}
static void on_dvr_reenc_bitrate(const char *value)
{
    int kbps = value ? atoi(value) : 0;
    if (kbps > 0)
        DVR_LIVE(dvr_reenc_set_bitrate(kbps), "dvr_reenc_set_bitrate(%d)", kbps);
}
...
static void on_webcam_fps(const char *value)
{
    int fps = value ? atoi(value) : 0;
    if (fps > 0)
        DVR_LIVE(webcam_set_fps(fps), "webcam_set_fps(%d)", fps);
}
static void on_webcam_quality(const char *value) /* JPEG quality 1..99 */
{
    int q = value ? atoi(value) : 0;
    if (q > 0)
        DVR_LIVE(webcam_set_quality(q), "webcam_set_quality(%d)", q);
}
...
static void notify_restart(const char *v)
{
    (void)v;
    show_restart_notice();
}
...
static void run_restart_pp(void *c)
{
    (void)c;
    sig_handler(1);
}
static void run_exit_pp(void *c)
{
    (void)c;
    sig_handler(2);
}

static void run_reboot_air(void *c)
{
    (void)c;
    colmenu_exec("gsmenu.sh button air actions Reboot");
}
static void run_reboot_gs(void *c)
{
    (void)c;
    colmenu_exec("gsmenu.sh button gs actions Reboot");
}
...
{
    wifi_net_t *net = ctx;
    if (!net->secured || net->saved[0])
    { /* open, or password already known */
        char cmd[512];
        if (net->saved[0])
...
static void wifi_rescan(void *ctx)
{
    (void)ctx;
    colmenu_rescan();
}
...
static void build_networks(colmenu_emit_t *e)
{
    ...
    free(saved);
    if (n == 0)
        colmenu_emit_label(e, "No networks found");
...
static void wifi_disconnect(void *ctx)
{
    (void)ctx;
    colmenu_exec("gsmenu.sh set gs wifi disconnect");
}
...
static void build_dvr(colmenu_emit_t *e)
{
    ...
    if (dvr_template)
    {
        ...
        if (sl)
        {
            size_t n = sl - dvr_template + 1;
            if (n < sizeof(dir))
            {
                memcpy(dir, dvr_template, n);
                dir[n] = '\0';
            }
        }
    }
    ...
    if (n == 0)
        colmenu_emit_label(e, "No recordings found");
}
...
static void build_apfpv_adapters(colmenu_emit_t *e)
{
    ...
    if (n == 0)
        colmenu_emit_label(e, "No wlx adapters found");
}
...
static void build_root(colmenu_emit_t *e)
{
    ...
    if (apfpv)
        colmenu_emit_submenu(e, LV_SYMBOL_WIFI, "APFPV", &gs_apfpv_page);
    else
        colmenu_emit_submenu(e, LV_SYMBOL_WIFI, "WFB-NG", &gs_wfbng_page);
